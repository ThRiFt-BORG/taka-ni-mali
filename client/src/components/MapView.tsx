...
// --- START: Your Existing Hardcoded Data and Icons (Unchanged) ---
const wasteData = {
...
// --- END: Your Existing Hardcoded Data and Icons ---
...
// --- Note: `markers` prop is currently unused in this version, but we keep it for future use. ---
export default function MapView({ onMarkerClick, selectedMarker }: MapViewProps) {
  ...
  useEffect(() => {
    if (mapContainer.current && !mapInstance.current) {
      ...
      fetch("/Kakamega County.geojson")
        ...
        .then((data: any) => {
          if (mapInstance.current) {
            ...
            aoiLayer.addTo(mapInstance.current);
          }
        })
      ...
      // --- THE FIX IS HERE ---
      // We iterate over the static `wasteData` and make each marker interactive.
      wasteData.features.forEach((feature) => {
        ...
        // Add a click event handler to each static marker.
        marker.on("click", () => {
          // When clicked, call the `onMarkerClick` function passed from the parent.
          // We create a "mock" MapMarker object because the dashboard only needs the siteName.
          const mockMarker: MapMarker = {
            id: 0, // Placeholder ID
            lat: coords[1],
            lng: coords[0],
            siteName: props.name,
            wasteType: '', // Placeholder
            volume: 0,     // Placeholder
            date: new Date(), // Placeholder
          };
          onMarkerClick(mockMarker);
        });
      ...
      if (formalCheckbox) {
        formalCheckbox.addEventListener("change", (e) => {
          if ((e.target as HTMLInputElement).checked) map.addLayer(layers.formal);
          else map.removeLayer(layers.formal);
        });
      }
      // (Repeat for informal, processing, plastic checkboxes...)
  ...
  }, [onMarkerClick]); // Added onMarkerClick to dependency array

  // This effect opens the popup of the selected marker
  useEffect(() => {
    const map = mapInstance.current;
    if (!map || !selectedMarker) return;

    map.eachLayer(m => {
        const marker = m as L.Marker & { feature?: any };
        // Check if this marker is one of our static markers
        if (marker.feature && marker.feature.properties.name === selectedMarker.siteName) {
            marker.openPopup();
